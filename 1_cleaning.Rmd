...
studies <- read_excel("1_input_data/studies_info.xlsx", sheet = "sample")
```

```{r real_data, echo=FALSE, message=FALSE, warning=FALSE}

bfa <- 
  read.dta13("3_country_data/bfa/BF_vaccine_data.dta")  %>%
  dplyr::as_tibble() %>%
  dplyr::mutate(
...
col <-  
  read.dta13("3_country_data/col/COL_vaccine_data.dta") %>% 
  dplyr::as_tibble() %>% 
...
ind <-
  read.dta13("3_country_data/ind/IGC_vacstudy_blfu12.dta", 
             nonint.factors = TRUE) %>%
  dplyr::as_tibble() %>% 
...
moz <-  
  read.dta13("3_country_data/moz/Survey_Data_Maputo__6_Mar_2021_18_33_33.dta", 
             nonint.factors = TRUE) %>%
  dplyr::bind_rows(read.dta13("3_country_data/moz/Survey_Data_Pemba__6_Mar_2021_18_33_35.dta", 
                              nonint.factors = TRUE)) %>%
  dplyr::as_tibble() %>% 
  dplyr::mutate(
...
nga <- 
  readr::read_csv("3_country_data/nga/Covidonlysheet.csv") %>%
  dplyr::rename(
...
npl <- 
  read.dta13("3_country_data/npl/vaccines_nepal.dta") %>%
  dplyr::as_tibble() %>% 
  dplyr::rename(
...
pak_1 <- 
  read.dta13("3_country_data/pak_1/Sheikhupura Policing Project_EVA Tracker_Vaccine Subdata_210223.dta") %>%
  dplyr::as_tibble() %>% 
  dplyr::rename(
    educ = i_0,
    gender = gender_o,
...
pak_2 <- 
  read.dta13("3_country_data/pak_2/CERP_EVA_Data.dta") %>%
...
load("3_country_data/rus/russia_vaccine_full.rdata")
rus <- 
  russia_vaccine %>%
  dplyr::rename(
    ...
    weight = wgt) %>%
  dplyr::mutate(
    take_vaccine = ifelse(D16takevaccine_1 == 1 | D16takevaccine_2 == 1, "Yes",  NA_character_),
...
rwa <- 
  read.dta13("3_country_data/rwa/RWA_vaccine_data.dta",
  ...
  dplyr::mutate(
    ...
    trust_vaccine_1 = 
      ifelse(trust_vaccine == "Family members", 1, 0),
    trust_vaccine_2 = 
      ifelse(trust_vaccine == "Friends you see and talk to" | trust_vaccine == "Friends you’ve made online", 1, 0),
    trust_vaccine_3 = 
      ifelse(trust_vaccine == "Religious leaders", 1, 0),
    trust_vaccine_4 = 
      ifelse(trust_vaccine == "Famous person", 1, 0),
...
sle_1 <- 
  read.dta13("3_country_data/sle_1/SLE_vaccine_data.dta",
             nonint.factors = TRUE) %>%
  as_tibble() %>% 
  dplyr::mutate(
    yes_vaccine_1 = as_numeric(yes_vaccine_1, start.at = 0),
...
sle_2 <-  
  read.dta13("3_country_data/sle_2/vaccine_subset.dta",
             nonint.factors = TRUE, generate.factors = TRUE) %>% 
  dplyr::as_tibble() %>% 
  dplyr::rename(
    trust_vaccine = vaccine_trust,
  ...
  dplyr::mutate(
    ...
    trust_vaccine_5 = 
      ifelse(trust_vaccine == "A doctor, nurse or other staff at a community health clinic" |
...
uga_1 <-  
  read.dta13("3_country_data/uga/IGC_Covid19vaccine_questions_final_clusterID.dta",
...
uga <- 
  read.dta13("3_country_data/uga_1/20210125_baseline_covid.1.2.3_merged_final.dta") %>% 
  dplyr::as_tibble() %>% 
...
load("3_country_data/usa/usa_covid_survey.RData")
usa <- 
  d %>%
  dplyr::as_tibble() %>% 
  dplyr::select(
  ...
  dplyr::mutate(
    ...
    no_vaccine_666 = 
      ifelse(WZB_no_vaccine_4 == 1 | WZB_no_vaccine_text_code %in% c("2", "5", "11", "3, 11", "888"),1,0),
...
vars <- readxl::read_excel("1_input_data/dictionary.xlsx") %>% pull(outcome)
...
readr::write_csv(df, "2_rep_data/combined.csv")
```
...
wgmdata <- readr::read_csv("1_input_data/wgm_2018_publiccsv.csv")
...
readr::write_csv(table_wgm, "2_rep_data/table_wgm.csv")
```
...
BCG  <- 
  readxl::read_xls("1_input_data/WHO_coverage_estimates_series.xls",sheet = "BCG") %>%
  select(Cname,`2019`) %>% rename(BCG = "2019")

MCV1 <- 
  readxl::read_xls("1_input_data/WHO_coverage_estimates_series.xls",sheet = "MCV1") %>%
  select(Cname,`2019`) %>% rename(MCV1 = "2019")

DTP1 <- 
  readxl::read_xls("1_input_data/WHO_coverage_estimates_series.xls",sheet = "DTP1") %>%
...
readr::write_csv(vacc_cov, "2_rep_data/vacc_cov.csv")
```