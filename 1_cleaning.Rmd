...
sle_1 <- 
  read.dta13("1_country_data/sle_1/SLE_vaccine_data.dta",
             nonint.factors = TRUE) %>%
  as_tibble() %>% 
  dplyr::mutate(
    yes_vaccine_1 = as_numeric(yes_vaccine_1, start.at = 0),
...
sle_2 <-  
  read.dta13("1_country_data/sle_2/vaccine_subset.dta",
             nonint.factors = TRUE, generate.factors = TRUE) %>% 
  dplyr::as_tibble() %>% 
  dplyr::rename(
    trust_vaccine = vaccine_trust,
...
uga_1 <-  
  read.dta13("1_country_data/uga/IGC_Covid19vaccine_questions_final_clusterID.dta",
...
vars <- readxl::read_excel("dictionary.xlsx") %>% pull(outcome)
