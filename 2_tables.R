...
nas <- df %>% group_by(country) %>%
  ...
  kableExtra::footnote(
    general_title = "",
    general = "Table 10 show the proportion of observations that are not missing values for each variable included in Figure 1.",
    threeparttable = T) 
...
# Call data from WGM
dfwgm <- read.csv("2_rep_data/table_wgm.csv")

# Call data from WHO
df_vacc_coveragebis <- read.csv("2_rep_data/vacc_cov.csv")
...
tab_question3 <- 
  ...
  kableExtra::footnote(
    general_title = "",
    general = "Table 14 presents question wording and answer options used in Figure 2 to get an estimated percentage of reasons not to take the COVID-19 vaccine. Columns 3-10 show the answer options that were recoded in each category. Answer options are separated by a semicolon." ,
    threeparttable = T) %>% 
  kableExtra::landscape()
...
#WGM data
wgmdata <- 
  readr::read_csv("1_input_data/wgm_2018_publiccsv.csv") %>% 
